...
export default async function SchedulePage({
  ...
  searchParams: Promise<{ day?: string; div?: string }>
}) {
  const { day, div } = await searchParams
  ...
  const [games, standings] = await Promise.all([getGames(tournamentId), getStandings(tournamentId)])
    ...
    <div>
      <div className="mb-6">
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <a href={'/schedule?div=men&day=' + activeSlug} style={activeDiv === 'men' ? { background: '#f9f6f0', color: '#1a1a1a', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' } : { background: '#2e2e2e', color: '#888', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' }}>Men</a>
          <a href={'/schedule?div=women&day=' + activeSlug} style={activeDiv === 'women' ? { background: '#f9f6f0', color: '#1a1a1a', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' } : { background: '#2e2e2e', color: '#888', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' }}>Women</a>
        </div>

        <div style={{ display: 'flex', gap: '8px' }}>
          {DAYS.map(d => (
            <a key={d.slug} href={'/schedule?div=' + activeDiv + '&day=' + d.slug} style={activeSlug === d.slug ? { background: '#f9f6f0', color: '#1a1a1a', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' } : { background: '#2e2e2e', color: '#888', padding: '6px 16px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' }}>{d.label}</a>
          ))}
        </div>
      </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium" style={{ color: homeWins ? '#1a1a1a' : '#555' }}>{game.home_team.name}</span>
                              {homeRecord && <span className="text-xs" style={{ color: '#bbb' }}>{homeRecord}</span>}
                            </div>
                          ...
                          <div className="flex items-center justify-between mt-0.5">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium" style={{ color: awayWins ? '#1a1a1a' : '#555' }}>{game.away_team.name}</span>
                              {awayRecord && <span className="text-xs" style={{ color: '#bbb' }}>{awayRecord}</span>}
                            </div>
